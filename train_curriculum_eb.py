"""
Identical to train_curriculum_onset.py except the pretraining scales are
transposed by `pretrain_shift` semitones (default 3 = Eb major), while the
fine-tuning target (NocturneRousseau) is kept in its original key.

Usage:
    python train_curriculum_eb.py --pretrain_steps 100000 --finetune_steps 400000
"""
...
class Args:
    ...
    shift_factor: int = 0          # applied to the fine-tuning target only
    pretrain_shift: int = 3        # semitone shift for pretraining scales (3 = Eb major)
...
def get_env(
    ...
    env = suite.load(
        ...
        shift=shift if shift is not None else args.shift_factor,
        task_kwargs=dict(
...
def pretrain_on_scales(
    ...
    print("\n" + "=" * 60)
    print(f"PHASE 1: PRETRAINING ON SCALES (shift={args.pretrain_shift} semitones = Eb major)")
    ...
    for cycle in range(args.pretrain_steps // (num_scales * steps_per_scale) + 1):
        for scale_idx, scale_env_name in enumerate(args.scale_environments):
            ...
            print(f"\nCycle {cycle + 1}, Scale: {scale_env_name} (+{args.pretrain_shift} semitones)")
            print(f"Training steps {steps_already_done + 1} -> {steps_this_round} (global: {current_step + steps_already_done} -> {segment_end})")

            # Pass pretrain_shift so scales are transposed; target keeps shift_factor=0.
            env = get_env(scale_env_name, args, args.seed + scale_idx, shift=args.pretrain_shift)
            eval_env = get_env(
                scale_env_name,
                args,
...
def finetune_on_target(
    ...
    print("\n" + "=" * 60)
    print("PHASE 2: FINE-TUNING ON TARGET (NocturneRousseau, original key)")
    ...
    # No shift override here — target stays in its original key.
...
def main(args: Args) -> None:
    if args.name:
        ...
        run_name = f"SAC-eb-curriculum-{args.seed}-{time.time()}"
    ...
    wandb.init(
        ...
        tags=(args.tags.split(",") if args.tags else []) + ["pretrain-finetune", "eb-major", "onset-alignment"],
